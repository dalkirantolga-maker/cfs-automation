...
COLUMNS = [
    ...
    "Truck No", "Driver Name", "Released By", "Kayıt Yapan",
    "PDF Dosya Yolu", "Son Güncelleme", "Not"
]
...
[data-testid="stSidebar"] {
    background: linear-gradient(180deg,#001B3A,#003B73);
}
[data-testid="stSidebar"] * {
...
def parse_do(text):
    data = {
        "Container No": "", "DO No": "", "BL No": "", "Acente": "",
        "Consignee": "", "Vessel": "", "Voyage": "", "Size/Type": "",
        "Seal No": "", "EXP Date": ""
    }
    ...
    if "CMA CGM" in upper_text:
        data["Acente"] = "CMA CGM"
    elif "MSC" in upper_text:
        data["Acente"] = "MSC"
    elif "MAERSK" in upper_text:
        data["Acente"] = "MAERSK"

    lines = [x.strip() for x in text.split("\n") if x.strip()]
    for line in lines[:25]:
        u = line.upper()
        if not any(x in u for x in ["DELIVERY", "PAGE", "VESSEL", "VOYAGE", "CMA", "MSC", "MAERSK", "B/L", "CONTAINER", "SEAL"]):
            if len(line) > 3:
                data["Consignee"] = line
                break
...
if menu == "Dashboard":
    ...
    card(c1, "📦 TOPLAM", total, "Tüm kayıtlar", "blue")
    card(c2, "⏳ BEKLEYEN", waiting, "İşlem bekliyor", "orange")
    card(c3, "✅ CFS AÇILDI", opened, "CFS işlemde", "green")
    ...
    with c2:
        ...
        temp = df.copy()
        temp["EXP Parsed"] = temp["EXP Date"].apply(parse_date_any)
        ...
        if result.empty:
            ...
            with c2:
                ...
                edit_status = st.selectbox(
                    "CFS Durumu",
                    statuses,
                    index=statuses.index(row["CFS Durumu"]) if row["CFS Durumu"] in statuses else 0
                )

                edit_damage = st.selectbox(
                    "Hasar Durumu",
                    damages,
                    index=damages.index(row["Hasar Durumu"]) if row["Hasar Durumu"] in damages else 0
                )
    ...
    if search:
        ...
        if not result.empty:
            pdf_rows = result[result["PDF Dosya Yolu"].astype(str) != ""]
            if not pdf_rows.empty:
                st.markdown("### 📎 PDF Bilgileri")
                st.dataframe(pdf_rows[["Container No", "DO No", "PDF Dosya Yolu"]], use_container_width=True)
    else:
